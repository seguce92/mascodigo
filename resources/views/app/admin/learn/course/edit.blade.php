  ...
  <form class="flex flex-wrap lg:flex-no-wrap w-full" action="{{ route('courses.update', $course->id) }}" method="POST">
    ...
    <div class="flex flex-wrap w-full lg:w-9/12 bg-white border rounded shadow p-2 mr-0 lg:mr-2">
      <div class="w-full">
        ...
        <input class="form-input" id="title" name="title" type="text" autofocus="true" placeholder="Titulo de Curso" onkeyup="generateSlug()" value="{{ old('title') ? old('title') : $course->title }}">
      </div>
      
      <div class="w-full lg:mb-1 md:mb-0 sm:mb-0">
        <label class="form-label normal-case" for="slug">URL PERMANENTE: {{ url('/') }}/</label>
        <input class="form-input" id="slug" name="slug" type="text" placeholder="{{ url('/') }}" value="{{ old('slug') ? old('slug') : $course->slug }}">
      </div>

      <div class="w-full">
        <label class="form-label" for="template">Plantilla de Certificado</label>
        <input class="form-input" id="template" name="template" type="text" placeholder="Plantilla para el certificado" value="{{ old('template') ? old('template') : $course->template }}">
      </div>
      ...
      <div class="relative w-full sm:w-1/1 md:w-1/3 lg:w-1/3 xl:w-1/3 mb-1 pr-1">
        ...
        <select name="skill_id" class="form-select" id="skill_id">
          @foreach ( $skills as $skill )
            <option value="{{ $skill->id }}" {{ $skill->id == $course->skill_id ? 'selected' : '' }}>{{ $skill->name }}</option>
          @endforeach
        </select>
      </div>
      <div class="relative w-full sm:w-1/1 md:w-1/3 lg:w-1/3 xl:w-1/3 mb-1 pr-1">
        ...
        <div class="relative w-4/5 mb-1 pr-1">
          ...
          <select name="color" onchange="changeColor()" id="color" class="form-select">
            @foreach ( config('seguce92.data.colors') as $color )
              <option value="{{ $color }}" {{ $color == $course->color ? 'selected' : '' }}>{{ $color }}</option>
            @endforeach
          </select>
        </div>
      ...
      <div class="w-full mb-1 pr-1">
        ...
        <textarea class="form-input h-32" name="content" id="content">{{ old('content') ? old('content') : $course->content }}</textarea>
      </div>
    </div>

    <div class="flex self-start flex-wrap w-full lg:w-3/12 bg-white border rounded shadow p-2">
      <div class="w-full mb-1">
        <label class="form-label" for="description">Icono / Imagen(SVG)</label>
        <file-upload id="image" value="{{ $course->icon }}" url="admin/media/icon/course" :options="{ mime: 'image/svg+xml' }"></file-upload>
      </div>
      <div class="w-full mb-1">
        <label class="form-label">estado de curso</label>
        <label class="inline-flex items-center mr-5">
          <input type="radio" class="form-radio h-6 w-6" name="is_publish" value="1" {{ $course->is_publish ? 'checked' : '' }}>
          <span class="ml-2">Publicar</span>
        </label>
        <label class="inline-flex items-center">
          <input type="radio" class="form-radio h-6 w-6" name="is_publish" value="0" {{ $course->is_publish == false ? 'checked' : '' }}>
