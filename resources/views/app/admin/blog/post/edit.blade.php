...
@section('content')
  <form class="flex flex-wrap lg:flex-no-wrap w-full" action="{{ route('posts.update', $post->id) }}" method="POST">
    @csrf
    @method('PATCH')
    <div class="flex flex-wrap self-start w-full lg:w-9/12 bg-white border rounded shadow p-2 mr-0 lg:mr-2">
      <div class="w-full">
        ...
        <input class="form-input" id="title" name="title" type="text" autofocus="true" placeholder="Titulo de Post" onkeyup="generateSlug()" value="{{ old('title') ? old('title') : $post->title }}">
      </div>
      
      <div class="w-full lg:mb-1 md:mb-0 sm:mb-0">
        <label class="form-label normal-case" for="slug">URL PERMANENTE: {{ url('/') }}/</label>
        <input class="form-input" id="slug" name="slug" type="text" placeholder="{{ url('/') }}" value="{{ old('slug') ? old('slug') : $post->slug }}">
      </div>
    ...
    <div class="flex flex-wrap self-start w-full lg:w-3/12 bg-white border rounded shadow p-2">
      <div class="w-full mb-1">
        ...
        <file-upload id="image" value="{{ $post->image }}" url="admin/media/icon/post" :options="{ mime: 'image/jpg,image/png,image/jpeg' }"></file-upload>
      </div>
      ...
      <div class="w-full mb-1">
        ...
        <label class="inline-flex items-center mr-5">
          <input type="radio" class="form-radio h-6 w-6" name="is_publish" value="1" {{ $post->is_publish ? 'checked' : '' }}>
          <span class="ml-2">Publicar</span>
        </label>
        <label class="inline-flex items-center">
          <input type="radio" class="form-radio h-6 w-6" name="is_publish" value="0" {{ !$post->is_publish ? 'checked' : '' }}>
