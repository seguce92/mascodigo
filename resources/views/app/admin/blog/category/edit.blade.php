    ...
    <form class="w-full" action="{{ route('categories.update', $category->id) }}" method="POST">
      ...
      <div class="flex flex-wrap">
        <div class="w-full">
          ...
          <input class="form-input" id="title" name="title" type="text" placeholder="Título" onkeyup="generateSlug()" value="{{ old('title') ? old('title') : $category->title }}">
        </div>
        
        <div class="w-full lg:mb-1 md:mb-0 sm:mb-0">
          <label class="form-label normal-case" for="slug">SLUG - URL PERMANENTE: {{ url('/') }}/</label>
          <input class="form-input" id="slug" name="slug" type="text" placeholder="{{ url('/') }}" value="{{ old('slug') ? old('slug') : $category->slug }}">
        </div>
