...
@section('content')
  <form class="flex flex-wrap lg:flex-no-wrap w-full justify-center" action="{{ route('channels.store') }}" method="POST">
    @csrf
    <div class="flex flex-wrap self-start w-full lg:w-9/12 bg-white border rounded shadow p-2 mr-0 lg:mr-2">
      <div class="w-full">
        ...
        <input class="form-input" id="title" name="title" type="text" autofocus="true" placeholder="Titulo de Canal" onkeyup="generateSlug()" value="{{ old('title') }}">
      </div>
      ...
      <div class="flex content-between sm:w-1/1 md:w-1/3 lg:w-1/3 xl:w-1/3 mb-1 pr-1">
        <div class="relative w-4/5 mb-1 pr-1">
          ...
          <select name="icon" onchange="changeIconColor()" id="icon" class="form-select">
            ...
            @foreach ( $keys as $key )
              <option value="{{ config('seguce92.data.hexadecimal.'.$key) }}">{{ $key }}</option>
            @endforeach
          </select>
        </div>
